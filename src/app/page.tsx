...
export default function Component() {
  ...
  const getButtonClasses = () => {
    let classes =
      "bg-white text-black hover:bg-gray-200 transition-colors duration-200 font-semibold rounded-full";
    if (windowWidth < 640) {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-black text-white">
      ...
      <div className="flex-1 flex flex-col justify-center items-center md:items-end p-4 md:p-8">
        <h1 className="text-4xl md:text-6xl font-bold mb-4 md:mb-8 text-center md:text-right">
          Portfolio 0_0
        </h1>
        <div className="w-full max-w-xs md:max-w-md flex justify-center md:justify-end">
          <button className={getButtonClasses()}>Sign up</button>
        </div>
      </div>
    </div>
  );
